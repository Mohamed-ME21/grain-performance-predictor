...
def load_reverse_assets(grain_type):
    ...
    if grain_type == "Bates":
        ...
        p = d / "Road and Tube"
        assets["model"] = tf.keras.models.load_model(str(p / "rod_inverse_model.keras"), compile=False)
        assets["s_X"]   = joblib.load(str(p / "rod_inv_scaler_dims.pkl"))
        assets["s_yt"]  = joblib.load(str(p / "rod_inv_scaler_thrust.pkl"))
        assets["s_yp"]  = joblib.load(str(p / "rod_inv_scaler_pressure.pkl"))
        assets["s_ys"]  = joblib.load(str(p / "rod_inv_scaler_scalars.pkl"))
        ...
        p = d / "X Models"
        assets["model"] = tf.keras.models.load_model(str(p / "x_grain_inverse_model.keras"), compile=False)
        assets["s_X"]   = joblib.load(str(p / "x_inv_scaler_dims.pkl"))
        assets["s_yt"]  = joblib.load(str(p / "x_inv_scaler_thrust.pkl"))
        assets["s_yp"]  = joblib.load(str(p / "x_inv_scaler_pressure.pkl"))
        assets["s_ys"]  = joblib.load(str(p / "x_inv_scaler_scalars.pkl"))
